...
def main() -> None:
    ...
    with open(corpus_root /'clean' / 'data.txt', 'r', encoding='utf-8') as src:
        proverbs = [line.rstrip('\r\n') for line in src]
    # Считаем статистику
    with open(corpus_root / 'statistics.txt', 'w', encoding='utf-8') as dst:
        print('Всего новостей:', len(proverbs), file=dst)

        words: List[str] = [w.lower().strip('.,') for p in proverbs for w in p.split()]
        ...
        print('Количество символов (включая пробелы):',
              sum(len(p) for p in proverbs), file=dst)
